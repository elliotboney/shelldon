...
- **Credential file blocklist gaps in `_deny_sensitive`** [`shelldon/worker/tools.py:_deny_sensitive`] — `.env.bak`, `.env.backup`, `.env.old`, `.pem`, `.key`, `id_rsa` etc. are not caught. Structural jail + vault-outside-workspace is the real defense; explicitly scoped as defense-in-depth. Trigger: Story 9.5 hardening or when credential file patterns broaden.
...
- **`_http_get` follows redirects without SSRF mitigation** [`shelldon/worker/tools.py:278` → Story 9.5] — `follow_redirects=True` with no scheme/host check on redirect targets. A redirect to `http://169.254.169.254/` (cloud metadata) or `http://localhost:6379` (Redis) is followed. The owner-approval gate is the primary defence; SSRF blocking on redirect targets is 9.5 hardening scope.
- **`_http_get` buffers full HTTP response before `_cap()` truncates** [`shelldon/worker/tools.py:278` → Story 9.5] — `resp.text` downloads the entire response into RAM before the 16 KB cap is applied. A multi-MB response risks OOM on the 416 MB Pi. Streaming with a pre-read byte cap belongs to 9.5 resource hardening.
- **`prune_expired_approvals` is never scheduled** [`shelldon/core/history.py:263`] — no scheduler job or startup call; expired rows accumulate in `pending_approvals` indefinitely. Low urgency — rows are consumed or expire passively. Register a REFLEX-tier scheduler job when 9.5 hardening pass runs.
- **`_run_shell` can orphan background processes spawned via `&`** [`shelldon/worker/tools.py:260` → Story 9.5] — shell commands that daemonize (`cmd &`, `disown`) exit the shell within the 20s timeout but leave children running. Needs `start_new_session=True` + process-group cleanup in 9.5.
- **`_git` lacks git subcommand allowlist** [`shelldon/worker/tools.py:267` → Story 9.5] — `git clone`, `--upload-pack`, `-c core.sshCommand=bash` and similar forms can exfiltrate data or spawn processes. An allowlist of safe subcommands (status, log, diff, add, commit, push) would gate this; owner-approval is the current sole defence.
