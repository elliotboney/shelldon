...
Surfaced running retro Action 4 (live-validate Epic 10 on `gotchi` vs live GLM). Action 4 itself = SUBSTANTIALLY VALIDATED: deploy + non-destructive copy-if-absent seeding (all 9 persona files seeded into the live root, `summary.md` byte-unchanged), `rewrite_soul` op wrote the injected `memory/SOUL.md` end-to-end, face redrew on the turn, and the per-turn GLM cache signal logged live. **GLM cache passthrough RESOLVED (the 10.5 unknown):** z.ai's Anthropic-compat proxy surfaces `cache_read_input_tokens` (returned `0`) but OMITS `cache_creation_input_tokens` (logged as `None`) — `getattr` guard handled it, no crash. read=0 on a cold turn is expected; a 2nd same-prefix turn would show whether GLM auto-caches the prefix (not yet measured).

- **Model edits persona via BOTH the `rewrite_*` op AND the `write_file` tool (redundant + wrong)** [`shelldon/persona/BOT_INSTRUCTIONS.md` prompt behavior] — told to update its soul, the model emitted `rewrite_soul` (correct → wrote injected `memory/SOUL.md`) AND ALSO called the Epic 9 `write_file` tool on `SOUL.md`. `write_file` is jailed to `~/.shelldon/workspace/` (a dir the prompt never reads) and is risky-tier, so it wrote a DEAD duplicate AND needlessly interrupted the owner for approval. Both BOT_INSTRUCTIONS (Epic 10 ops awareness) and the Epic 9 tool surface are advertised, so the model reaches for both. Fix candidate: nudge BOT_INSTRUCTIONS to say persona files (`SOUL/IDENTITY/USER/about/BOT_INSTRUCTIONS`) are edited ONLY via their `rewrite_*` ops, never `write_file`/`read_file` (those are for the `workspace/` jail). Also confirmed read_file can't see memory (`read_file SOUL.md` → FileNotFoundError, workspace jail) — correct by design, but the model tried it. Trigger: any persona self-edit.
...
- **DESIGN QUESTION — SOUL/IDENTITY ship EMPTY; a bot arguably should have STARTER content** [`shelldon/persona/SOUL.md`, `IDENTITY.md`, `core/memory.py` 10.1 decision] — 10.1 deliberately ships `SOUL/IDENTITY/USER` empty (day-one byte-parity + onboarding fills them). Owner (2026-06-27): a bot should boot with a baseline soul/identity, not blank files. Reconsideration: ship STARTER templates for `SOUL.md` + `IDENTITY.md` (baseline character matching BOT_INSTRUCTIONS' voice), which the bot then EVOLVES — this also mitigates the clobber risk (always content to carry forward). KEEP `USER.md` EMPTY: it's the per-owner profile, the onboarding sentinel gates on USER-blank (10.4), and a starter USER would be presumptuous + break onboarding. Net: starter SOUL/IDENTITY, empty USER. Likely a small Epic 10 follow-on or Epic 11 item — owner to scope.
