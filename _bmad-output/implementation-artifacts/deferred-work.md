...
- **⚠️ TOP QUALITY BUG — Forked-worker history read hits a sqlite "locking protocol" error EVERY turn on the Pi** [`shelldon/worker/prompt.py` `gather_context` → `open_readonly`] — confirmed RECURRING on `gotchi` (2026-06-20, every turn, incl. live Telegram chat): `history read failed during assembly (locking protocol); degrading`. The 4.4 fail-soft keeps the turn working (replies + applies memory-ops fine), BUT the worker never reads recent-history/FTS5-recall → **shelldon has no short-term conversational memory on the Pi** (forgets what you just said across turns). Did NOT reproduce on macOS. Likely a WAL read-only-handle interaction on the Pi's sd-card filesystem (the forked worker opens history read-only while core holds the WAL writer). **Fix before "daily driver" — the pet appears to have amnesia mid-conversation.** Hypotheses to try: `PRAGMA busy_timeout` on the read-only handle; ensure WAL mode + `journal_mode` compatibility for the read-only open; or pass the recent-history into the worker from core (core already has it) instead of a second sqlite handle in the fork.
