...
- **[Action 1 — immediate] Close 10.4 + 10.5 review.** Fix 10.4's OPEN AC5 patch: `read_bootstrap` (`shelldon/core/memory.py`) swallows a non-UTF-8 corruption silently, so `_safe_read`'s except never fires and the spec's "onboarding section omitted (logged)" is unmet — add a `log.warning` on the corrupt-read path. Then run 10.4's formal code review and flip 10.4 + 10.5 `review → done` in sprint-status. Dangling review closures.
