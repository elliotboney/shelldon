...
## Epic 10 retrospective action items (2026-06-26)

From `epic-10-retro-2026-06-26.md`. Owner: Elliot. These are the carried commitments from the Epic 10 retro.

- **[Action 1 — ✅ DONE 2026-06-26] Close 10.4 + 10.5 review.** RESOLVED: added `log.warning("BOOTSTRAP.md unreadable …")` in `read_bootstrap`'s except (`shelldon/core/memory.py`); `test_onboarding_fail_soft_when_bootstrap_corrupt` now asserts the caplog warning; 10.4's last OPEN patch checked; 10.4 + 10.5 flipped `review → done`; epic-10 → done. 839 tests, import-linter 3 KEPT. (10.4 was already formally reviewed — 3 findings, 2 deferred — so only the OPEN patch needed closing, not a fresh review pass.)
- **[Action 2 — process] Operationalize the fail-soft-boundary checklist.** Epic 9's retro asked for "broad-`except` / silent-degrade as a first-class review dimension"; Epic 10 proved it generalizes to any fail-soft file boundary and it was STILL the dominant patch class (4 of 5 stories). Before marking any file-touching story done, scan for: broad `except`; degrade-without-a-log; re-seed-masks-delete in tests (test fail-soft on seeded files with CORRUPTION, not deletion — `CuratedMemory` re-seeds absent files on construction); missing-template-slot silent drop; owner-edit foot-guns. Add to the dev-loop checklist; make it pre-review, not post-review.
- **[Action 3 — test hardening] Harden the untracked-seed guard.** `tests/test_packaging.py:test_persona_seeds_are_git_tracked` is the sole catcher of an untracked seed (importlib/wheel tests pass on present-but-untracked files) and `pytest.skip`s when git is absent. Assert git is present in CI, or add a build-from-clean-clone check. (Duplicates the 10.5-review deferral above; consolidated here as a retro action.)
- **[Action 4 — live validation] Live-validate Epic 10 on the Pi.** One real owner conversation on `gotchi` vs live GLM exercising the whole epic: first-run onboarding (BOOTSTRAP) → autonomous persona rewrite (SOUL/IDENTITY/USER) → "how do you work" lazy-load (TOOLS/ARCHITECTURE inject) → read the per-turn GLM cache signal from the broker log (also resolves the 10.5 GLM `cache_control` passthrough unknown). Owner-run, out-of-CI, like the 8.0 live smokes. This is the production gate CI structurally cannot give.

## Epic 10 live-Pi validation findings (2026-06-27)

Surfaced running retro Action 4 (live-validate Epic 10 on `gotchi` vs live GLM). **Action 4 = ✅ FULLY VALIDATED 2026-06-27:** deploy + non-destructive copy-if-absent seeding (all 9 persona files seeded into the live root, `summary.md` byte-unchanged), face redrew on the turn, and — after the starter-template + write_file-nudge fix (ff4c4c1) was retrofitted live — onboarding completed cleanly: the bot interviewed, wrote `USER.md` via the `rewrite_user` OP (not `write_file`, no approval prompt), and the gate latched off. The full corrected loop (seed → owner-focused onboard → persona-write via the op path → latch off → starter identity in place) runs on real hardware. **GLM cache passthrough RESOLVED (the 10.5 unknown):** z.ai's Anthropic-compat proxy surfaces `cache_read_input_tokens` (returned `0`) but OMITS `cache_creation_input_tokens` (logged as `None`) — `getattr` guard handled it, no crash. read stayed `0` across multiple turns → GLM does NOT surface prefix-cache hits over the proxy, so the lazy-load + 5.2 budget posture stands (no silent cap).

- **[✅ DONE ff4c4c1] Model edits persona via BOTH the `rewrite_*` op AND the `write_file` tool (redundant + wrong)** — RESOLVED via BOT_INSTRUCTIONS nudge ("edit persona via `rewrite_*` ops, NEVER `write_file`/`read_file` — those reach only `workspace/`"). Validated live 2026-06-27: the next persona write (`rewrite_user` at onboarding) used the op cleanly, no `write_file`, no approval prompt. Original finding: told to update its soul, the model emitted `rewrite_soul` (correct → wrote injected `memory/SOUL.md`) AND ALSO called the Epic 9 `write_file` tool on `SOUL.md`. `write_file` is jailed to `~/.shelldon/workspace/` (a dir the prompt never reads) and is risky-tier, so it wrote a DEAD duplicate AND needlessly interrupted the owner for approval. Both BOT_INSTRUCTIONS (Epic 10 ops awareness) and the Epic 9 tool surface are advertised, so the model reaches for both. Fix candidate: nudge BOT_INSTRUCTIONS to say persona files (`SOUL/IDENTITY/USER/about/BOT_INSTRUCTIONS`) are edited ONLY via their `rewrite_*` ops, never `write_file`/`read_file` (those are for the `workspace/` jail). Also confirmed read_file can't see memory (`read_file SOUL.md` → FileNotFoundError, workspace jail) — correct by design, but the model tried it. Trigger: any persona self-edit.
- **`rewrite_soul`/`rewrite_identity`/`rewrite_user` are FULL replacements — no guard against clobbering accumulated content** [`shelldon/core/memory.py:_apply_rewrite_persona`] — the op overwrites the whole file with the model's `content`; the only thing preserving prior soul/identity is the model choosing to carry it forward (it reads the current file each turn). On a fresh/empty file (this run) there's nothing to lose, but on a populated file a careless rewrite silently drops accumulated character. No structural mitigation today. Candidates: (a) starter-template seeding (below) so there's always baseline content the model evolves rather than a blank slate; (b) a soft guard/heuristic (e.g. warn/reject if a rewrite shrinks the file dramatically); (c) prompt nudge to "evolve, don't replace". Owner-raised concern.
- **[✅ DONE ff4c4c1] SOUL/IDENTITY ship EMPTY; a bot should have STARTER content** — RESOLVED: `SOUL.md` + `IDENTITY.md` seeds now ship with starter content (the bot evolves them); `USER.md` stays blank (the onboarding trigger); `BOOTSTRAP.md` refocused on the owner. Tests updated to the new day-one shape (abandons the obsolete byte-parity premise). Shipped + retrofitted live on the Pi. Owner-raised 2026-06-27.

## Epic 11 candidates (from Epic 10 live validation, 2026-06-27)

- **Injected prompt sections aren't addressed to their file/op — model must infer where to write** [`shelldon/worker/prompt.py:assemble_prompt`] — each memory source is injected under a SEMANTIC header (`# Your owner`, `# Your soul`, `# What you know`) but the header never names the backing file or the write-op. The section→file→op mapping lives separately in `BOT_INSTRUCTIONS.md`, so the model bridges it implicitly — the failure mode is mis-routed writes (owner facts → `remember(facts)` instead of `USER.md`; self-notes → `about.md` vs `SOUL.md`), worsened by genuine store overlap (USER vs facts vs preferences vs about). Fix: label each writable section inline with its file + op, e.g. `# Your owner — USER.md (update with rewrite_user)`, so provenance + destination + op are visible at the point the content appears (optionally + a memory-map table in BOT_INSTRUCTIONS). Small `assemble_prompt` change + header-string test updates. Owner-raised; logged for Epic 11.
- **`rewrite_soul`/`rewrite_identity`/`rewrite_user` full-replacement clobber — no STRUCTURAL guard** [`shelldon/core/memory.py:_apply_rewrite_persona`] — softened this session by starter content (always something to carry forward) + the BOT_INSTRUCTIONS "evolve, don't replace" nudge, but there is still no code-level mitigation against a careless rewrite wiping accumulated character on a populated file. Candidate: a soft guard (warn/reject if a rewrite drops the file dramatically in size, or keep a prior-version backup). Logged for Epic 11.
