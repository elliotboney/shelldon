...
- **`python -m shelldon` (multiprocess+`spawn`) CLI transport child can't read the parent's stdin** [`shelldon/app.py:126-127` `_transport_proc` / `launch_multiprocess`] — the transport runs as a `spawn` child whose stdin is NOT the launching process's pipe, so a piped owner message never reaches `run_cli_transport` (no turn fires; the 5 processes still spin up fine, peaked 348MB on the Pi, no OOM). The CLI-over-stdin transport only works in the **in-process** launcher. **This is really a "the only transport is stdin-CLI" gap** — production needs a non-stdin chat transport (Telegram/socket adapter, like v1 had). Blocks proving the full `launch_multiprocess` path end-to-end. → **scope as Story 8.2 (real chat transport)** — needed for the multiprocess path AND for desk usability. NOT a bug in the multiprocess launcher itself (it came up correctly).
- **Forked-worker history read hits a sqlite "locking protocol" error and degrades on the Pi** [`shelldon/worker/prompt.py` `gather_context` → `open_readonly`] — first observed on `gotchi` (2026-06-20): `history read failed during assembly (locking protocol); degrading`. The 4.4 fail-soft caught it (the turn still replied + applied the `remember`), so no user-visible failure — but the recent-history + FTS5 recall context was dropped for that turn. Likely a WAL / read-only-handle interaction on the Pi's filesystem (first turn / fresh db, or sd-card fsync timing). Root-cause before relying on recall in production — if it recurs every turn, the pet effectively has no short-term memory on the Pi despite the db being written.
